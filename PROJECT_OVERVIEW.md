...
- GitHub Actions AISA skill 代码回归：hosted lane 现在支持通过手动参数 `run_aisa_api_regression=true` 或仓库变量 `AUTO_RUN_AISA_API_REGRESSION=true` 运行 `scripts/test_aisa_api_skills.py`，生成并上传 `targets/aisa-api-regression-report-YYYY-MM-DD.json` artifact；开启时必须配置 `AISA_API_KEY`，缺失会 fail fast，第三方网络瞬断或单命令超时只记为 `transient_count`；CI 默认用 `45s` timeout、`0` retry 和短 delay 收紧耗时，并逐条输出进度日志；回归 smoke/report 产物不会再误触发 publish continuation 或自动推进 `main`
