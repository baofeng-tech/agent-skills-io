...
- GitHub Actions AISA skill 代码回归：hosted lane 现在支持通过手动参数 `run_aisa_api_regression=true` 或仓库变量 `AUTO_RUN_AISA_API_REGRESSION=true` 运行 `scripts/test_aisa_api_skills.py`，生成并上传/提交 `targets/aisa-api-regression-report-YYYY-MM-DD.json`；开启时必须配置 `AISA_API_KEY`，缺失会 fail fast，第三方网络瞬断只记为 `transient_count`；CI 默认用 `AUTO_AISA_API_REGRESSION_TIMEOUT` / `AUTO_AISA_API_REGRESSION_RETRIES` / `AUTO_AISA_API_REGRESSION_DELAY` 收紧耗时，并逐条输出进度日志
