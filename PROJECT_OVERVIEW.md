...
- ClawHub 2026-05-07 Review 判定修复：`scripts/clawhub_live_status.py` 与 diagnosis 现在把 ClawScan `Review` verdict 视作 suspicious blocker；`stock-dividend` 的 ClawHub 专用 slug 收敛为 `stock-dividend-aisa`，并在母版中补齐只读/无交易/无购买边界与 ticker 输入校验
- ClawHub 批量发布退出码修复：备用 token 登录或 `whoami` 超时时，如果目标 artifact 已由其它 token 发布且队列清空，`scripts/publish_clawhub_batch.py` 不再返回失败；新增 `scripts/test_clawhub_batch_publish_exit.py` 与 `scripts/test_clawhub_live_status_review.py`
