...
- 这个上游仓库对本仓库来说是**只读基线**
- 只能拉取、对比、吸收到本仓库
- 不能从本仓库自动化把 `targetSkills/`、README、`SKILL.md` 或任何生成层内容再提交回去
...
- `targets/github-actions-and-aisa-usage-review-2026-05-12.md`
  - 本轮复核最新 Actions 失败根因、确认 scheduled workflow 使用 `AISA_API_KEY` 的 LLM refinement / AISA 回归路径，并把定时变量切到省钱安全档
...
- 面向本仓库母版索引与后续下游生成
- 不再作为 `AIsa-team/agent-skills` 的自动回写发布索引
...
- `publish-targetSkills-to-agent-skills.sh`
  - 兼容旧入口名的 agentskill.sh 发布别名
  - 实际会重建并同步 `agentskill-sh-release/` 到 `baofeng-tech/agent-skills`
- `publish-claude-release.sh`
...
跨平台母版输出层。  
这里放的是已经整理成新结构的 skill，是本仓库的母版 source of truth。

用途：

- 作为跨平台主版本
- 作为 GitHub / AgentSkills / Hermes / Claude Code 的基础版本
- 作为各个平台 release layer 的唯一生成源
...
- ClawHub 2026-05-07 Review 判定修复：`scripts/clawhub_live_status.py` 与 diagnosis 现在把 ClawScan `Review` verdict 视作 suspicious blocker；`stock-dividend` 的 ClawHub 专用 slug 已从历史 `stock-dividend-slot1` 重命名为 `stock-dividend-aisa`，`1.0.2` live latest 现为 `vt=clean/clawscan=clean/static=clean`，母版补齐只读/无交易/无购买边界与 ticker 输入校验
- ClawHub plugin 元数据修复：`scripts/build_clawhub_plugin_release.py` 现在为 AISA-backed native plugin 写入 `providers`、`providerAuthEnvVars`、`providerAuthChoices`、`configUiHints`、`bundledSkills`、`commands` 与 `openclaw.environment`，避免 package capabilities / registry Requirements 区域低报 `AISA_API_KEY` / python / bash 运行面；`last30days-zh-plugin@1.0.9` 已发布为 latest，ClawScan / OpenClaw / Static analysis 均为 `clean`，仅 VirusTotal 仍为异步 `pending`
- ClawHub 2026-05-07 bibaofeng 定向修复：已按 rescan-first 流程处理用户点名 URL；`aisa-tavily-search-plugin@1.0.3`、`last30days-plugin@1.0.4`、`aisa-twitter-research-engage-relay-plugin@1.0.6` 与 `aisa-perplexity-search-sonar-plugin` 当前 live status 均为 `vt=clean/clawscan=clean/static=clean`；`twitter-post-aisa` 已用原 slug 发布到 `1.0.3`，`clawhub inspect` 显示 latest `1.0.3` 且 moderation 为空，live status 已从 suspicious 降为 `vt=pending/clawscan=clean/static=clean`；`twitter-aisa` 与 `@clawhub/prediction-market-arbitrage` inspect clean；`@clawhub/aisa-twitter-api` rescan 已请求但仍为平台异步 `pending`
- ClawHub 2026-05-08 点名 URL 复核：`twitter-aisa` rescan 达上限后已用原 slug 发布到 `1.0.5`，live status 为 `vt=clean/clawscan=clean/static=clean`；`last30days-slot2-plugin` 已用原 package name 发布到 `1.0.4`，当前 `scanStatus=pending` 且 live status 为 `vt=pending/clawscan=pending/static=clean/suspicious=0`；`@clawhub/aisa-twitter-api` rescan 已请求但精确重发被平台拒绝 `Forbidden for "@clawhub"`，仍是 scoped publisher 权限阻塞
- ClawHub plugin suspicious 精修：`aisa-tavily-search-plugin` 的 ClawHub plugin payload 现在在生成层把嵌入 JS 脚本改为显式 `--aisa-api-key` 参数，避免公开 plugin 包直接读取 `process.env` 触发 `suspicious.env_credential_access`；`last30days` / `last30days-zh` 的 ClawHub release 生成 allowlist 现在透传 `GH_TOKEN`、`GITHUB_TOKEN` 与 last30days 可选运行参数，避免 GitHub 可选凭据声明不一致
- ClawHub 批量发布退出码修复：备用 token 登录或 `whoami` 超时时，如果目标 artifact 已由其它 token 发布且队列清空，`scripts/publish_clawhub_batch.py` 不再返回失败；新增 `scripts/test_clawhub_batch_publish_exit.py`、`scripts/test_clawhub_live_status_review.py` 与 `scripts/test_clawhub_plugin_auth_metadata.py`
...
- GitHub Actions AISA skill 代码回归：hosted lane 支持通过手动参数 `run_aisa_api_regression=true` 或仓库变量 `AUTO_RUN_AISA_API_REGRESSION=true` 运行 `scripts/test_aisa_api_skills.py`，生成并上传 `targets/aisa-api-regression-report-YYYY-MM-DD.json` artifact；该步骤会消耗付费 `AISA_API_KEY` 流量，当前默认关闭，开启时必须配置 `AISA_API_KEY`；第三方网络瞬断或单命令超时只记为 `transient_count`；CI 默认用 `45s` timeout、`0` retry 和短 delay 收紧耗时，并逐条输出进度日志；回归 smoke/report 产物不会再误触发 publish continuation 或自动推进 `main`
...
- GitHub Actions 三条流程线：现在已经拆成（1）正常 upstream sync + 下游平台发布线，（2）suspicious diagnosis-driven repair 线，（3）breakout rollout 线；三条线在 workflow 中是独立 continuation job，不再混在同一个发布 job 里；workflow_dispatch 默认 `auto` 判断是否请求，也可手动设为 `true` 或 `false`
...
- GitHub Actions self-hosted 凭据回退：当 `DOWNSTREAM_REPO_TOKEN` 与 ClawHub tokens 等 CI secrets 未配置时，当前 runner 会回退读取本机 `/mnt/d/workplace/agent-skills-io/example/accounts`；下游仓库 clone/fetch/push 现在会使用 `DOWNSTREAM_REPO_TOKEN` 的 HTTPS URL，避免 SSH 超时或私有 repo clone 阻塞在发布前置阶段
- GitHub Actions 无上游变化续发：当 upstream 没有新增变更但 self-hosted publish / downstream sync 被打开时，`scripts/unified_skill_pipeline.py` 会从当前 `targetSkills/` 继续 rebuild、sync、publish 和 diagnosis，不再提前返回
- ClawHub publish state 路径收敛：`scripts/publish_clawhub_batch.py` 现在把 artifact `path` 记录为 repo-relative 路径，避免 dry-run 或不同机器执行时把本机绝对路径写进 `targets/clawhub-publish-state.json`
...
**这是一个把历史 OpenClaw / ClawPub 技能资产，重构成 AIsa 跨平台母版 skill 与多平台发布变体的中间工厂。**
