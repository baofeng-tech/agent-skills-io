...
- ClawHub plugin 元数据修复：`scripts/build_clawhub_plugin_release.py` 现在为 AISA-backed native plugin 写入 `providers`、`providerAuthEnvVars`、`providerAuthChoices` 与 sensitive `uiHints`，避免 registry Requirements 区域低报 `AISA_API_KEY` / python / bash 运行面；`last30days-zh-plugin` 因此推进到 `1.0.8` 待发布/复扫
