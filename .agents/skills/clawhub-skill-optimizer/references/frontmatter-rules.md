...
## Validation constraints by mode

These rules differ depending on the target artifact:

### Cross-platform mode

- Optimize for AgentSkills-style compatibility.
- Typical top-level keys may include `name`, `description`, `license`, `allowed-tools`, and `metadata`.
- Additional metadata siblings are acceptable when they are useful to Hermes or Claude Code and do not break the target runtime.

### ClawHub publish mode

- Optimize for conservative upload compatibility.
- Prefer a flat shape with top-level `name`, `description`, `author`, `version`, `license`, `user-invocable`, `primaryEnv`, and `requires` when needed.
- Keep `metadata` focused on `metadata.openclaw` only.
- Avoid unrelated `metadata` siblings such as `metadata.hermes`, broad tag bags, or homepage-only decoration when publishing.

## Legacy validator note

Some older internal validators assume only `name`, `description`, `license`, `allowed-tools`, and `metadata` at the top level. Treat that as a narrow parser profile, not a universal ClawHub truth. If a specific validator is in play, match it; otherwise prefer the explicit publish-mode shape above for ClawHub uploads.
...
license: [license]
allowed-tools: [tool list when applicable]
metadata:
  author: [author]
  version: "[version]"
  openclaw:
