...
- self-hosted publish, suspicious repair, and breakout rollout now pass through a hosted preflight first; if those lanes are requested and no online self-hosted runner matches `SELF_HOSTED_RUNNER_RUNS_ON_JSON` (internal default `["self-hosted"]`), preflight can run them on `ubuntu-latest` when hosted continuation fallback is enabled, or fail fast with a summary when fallback is disabled
- `SELF_HOSTED_RUNNER_RUNS_ON_JSON` should be omitted/deleted when there is no registered self-hosted runner or when the default `["self-hosted"]` selector is enough; only set it when a real runner needs extra labels such as `["self-hosted","linux","clawhub"]`
...
- hosted and self-hosted Python bootstrap now installs the lightweight packages used by read-only AISA regression (`openai`, `httpx`, and `requests`) in addition to `PyYAML`, so a configured `AISA_API_KEY` does not turn into false failures from missing optional imports
...
2. installs Python 3.12 plus `PyYAML`, `openai`, `httpx`, and `requests`
...
Use `SELF_HOSTED_RUNNER_RUNS_ON_JSON` as the single source for runner labels, for example `["self-hosted","linux","clawhub"]`. The preflight parser and the actual self-hosted `runs-on` target both read this JSON value. It does not create or start a runner; it only states which labels a registered online runner must have.
